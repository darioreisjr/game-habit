...
export default async function HomePage() {
  ...
  return (
    <MapView
      stats={stats || { user_id: user.id, level: 1, xp: 0, coins: 0, updated_at: new Date().toISOString() }}
      profile={{ id: user.id, name: displayName, created_at: profile?.created_at || new Date().toISOString() }}
      habits={todayHabits || []}
      checkins={checkins || []}
    />
  );
